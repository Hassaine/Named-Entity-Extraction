...
class corpusManager:
    ...
    def getCorpusJSON(self,corpusName='NELexicon.txt'):
        words=[]
        for corpus in self.emissionSources:
            if(corpus.endswith(corpusName)):
                lines = open(corpus,'r',encoding='windows-1256').readlines()
                for line in lines:
                    line = line.split()
                    if(len(line)>1):
                        wordObject={'word':'','tags':[]}
                        tags=[]
                        wordObject['word']=line[0]
                        for tag in line[1:]:
                            tags.append(tag)
...
if __name__=='__main__':
    AM = corpusManager()
    #AM.loadCorpusNames()
    corpus=AM.getCorpusJSON('ArNameLexicon.txt')
    print(corpus[-1])
