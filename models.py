...
class HMM(Model):
    ...
    def buildEmissionMatrix(self,taggedCorpus:list,train_size):
        ...
        train=random.shuffle(taggedCorpus[:int(train_size*len(taggedCorpus))])
         #construction of the emission matrix
        emission = defaultdict(dict)
        for tag in NE_TAG_lABELS:
            emission[tag]=defaultdict(float)
        for tuple in train:
            ...
            emission[tuple[1].upper()][tuple[0]]+=1
    ...
    def buildTransitionMatrix(self,tagged_corpus:list,train_size):
        train=random.shuffle(tagged_corpus[:int(train_size*len(tagged_corpus))])
            #construction of the transition matrix
        transition = defaultdict(dict)
        for (tag1,tag2) in train:
            
            if tag1 not in transition:
                transition[tag1]=defaultdict(float)
    ...
    def train(self,tagged_corpus,test_size=0.2):
        self.ne_set=[el for el in tagged_corpus if el[1] in NE_TAG_lABELS]
        self.tag_set=list(bigrams([el[1] for el in tagged_corpus]))

        self.buildEmissionMatrix(self.ne_set,1-test_size)
        self.buildTransitionMatrix(self.tag_set,1-test_size)
    ...
    def loadTables(self):
        ...
        if not bool(self.EMISSION_MATRIX):
            if not os.path.exists('obj/hmm/'+self.emissMatrix_file_save_name+'.json'):
                ...
                saveComplexIndexjson(self.EMISSION_MATRIX, "obj\\hmm\\"+self.emissMatrix_file_save_name+'.json')
            else:
                self.EMISSION_MATRIX=loadComplexIndexJson("obj/hmm/"+self.emissMatrix_file_save_name+'.json')
        ...
        if not bool(self.TRANSITION_MATRIX):
            if not os.path.exists('obj/hmm/'+self.transMatrix_file_save_name+'.json'):
                ...
                saveComplexIndexjson(self.TRANSITION_MATRIX, "obj\\hmm\\"+self.transMatrix_file_save_name+'.json')
            else:
                self.TRANSITION_MATRIX=loadComplexIndexJson("obj/hmm/"+self.transMatrix_file_save_name+'.json')
                try:
...
if __name__=='__main__':
    fileName="../corpus/samples/test_sample.txt"
    HmmModel=HMM()
    ...
    #tagged=trigHMM.tagText(open(fileName, "r", encoding="windows-1256").read())[:200]
    tagged=HmmModel.tagText(open(fileName, "r", encoding="windows-1256").read())[:200]
